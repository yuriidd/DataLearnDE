...
У Docker'а есть Images (образы). Образ - это Linux дистрибутив, в котором нет ничего лишнего. Например, просто операционная система ubuntu с минимальным набором пакетом и команд для старта, все остальное можете поставить самостоятельно после запуска. 
...
Качаем образ, на котором будет строить наше приложение.
...
- Создать новый образ, используя docker compose, и запустить его
